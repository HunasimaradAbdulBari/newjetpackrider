...
const MainGameFile = () => {
    ...
    const initGame = async () => {
      try {
        ...
        if (isMobile) {
          ...
          // Desktop sizing logic (unchanged)
          ...
          gameWidth = Math.max(gameWidth, 640);
          gameHeight = Math.max(gameHeight, 320);
        }
  ...
  const createRedesignedJetpackScene = () => {
    return class JetpackGameScene extends window.Phaser.Scene {
      ...
      init(data) {
        ...
        // Dynamic scale factor based on actual screen dimensions
        const gameWidth = this.sys.canvas.width;
        const gameHeight = this.sys.canvas.height;
        
        if (this.isMobile) {
          // Mobile: scale based on actual device dimensions
          const baseWidth = 800; // smaller base for mobile
          ...
          // Desktop: original scaling logic
          const baseWidth = 1400;
          const baseHeight = 700;
          this.scaleFactor = Math.min(gameWidth / baseWidth, gameHeight / baseHeight);
      ...
      createRedesignedAssets() {
        ...
        // SVG Heart icons - responsive sizing
        ...
        // Question coin - responsive sizing
        const coinSize = Math.max(36, 48 * this.scaleFactor);
        const questionCoinGraphics = this.add.graphics();
        questionCoinGraphics.lineStyle(4, 0xEAB308, 1); 
        questionCoinGraphics.fillStyle(0xFDE047, 1);
        ...
        const maxHeight = gameHeight * 0.4;
        ...
        for (let x = 0; x <= gameWidth; x += buildingWidth + buildingSpacing) {
          ...
          const sx = Math.max(14, 18 * this.scaleFactor);
          const sy = Math.max(20, 25 * this.scaleFactor);
      ...
      createRedesignedPlayer() {
        ...
        const playerY = this.sys.canvas.height * 0.5;
      ...
      createHeartIcons() {
        ...
        const startX = Math.max(20, 30 * this.scaleFactor);
        const startY = Math.max(25, 35 * this.scaleFactor);
        for (let i = 0; i < 3; i++) {
      ...
      createHorizontalFuelBar() {
        ...
        const barHeight = Math.max(3, 4 * this.scaleFactor);
      ...
      setupInput() {
        ...
        this.input.on('pointerdown', (pointer) => {
          ...
          this.tweens.add({ 
            targets: ripple, 
            radius: Math.max(30, 40 * this.scaleFactor), 
            alpha: 0, 
            duration: 400, 
            ease: 'Power2', 
      ...
      update() {
        if (this.spaceKey.isDown || this.jetpackActive) {
          ...
          if (this.hasStartedFlying) {
            ...
            const targetY = this.hoverTargetY;
        ...
        if (this.gameState === 'QUESTION_ACTIVE') {
          this.updateMovingQuestionCoins(); 
          this.checkCoinCollisions();
        }
      }

      createJetpackParticles() {
        const offsetDistance = Math.max(6, 8 * this.scaleFactor);
        ...
        offsets.forEach(offset => {
          for (let i = 0; i < 2; i++) {
            ...
            const particle = this.add.circle(
              ...
              this.player.y + Math.max(30, 40 * this.scaleFactor), 
              particleSize,
        ...
        if (Math.random() < 0.6) {
          ...
          offsets.forEach(offset => {
            ...
            const fire = this.add.circle(
              ...
              this.player.y + Math.max(28, 38 * this.scaleFactor), 
              particleSize,
      ...
      cleanupObjects() {
        this.questionCoins = this.questionCoins.filter(coin => {
          if (coin.sprite.x < -100) {
            coin.sprite.destroy(); 
            if (coin.label) coin.label.destroy(); 
            return false;
          }
          return true;
        });
      ...
      showRedesignedQuestion() {
        ...
        this.time.delayedCall(1000, () => { 
          this.spawnMovingCoins(question); 
  ...
  this.time.delayedCall(800, () => this.showCleanInstruction());
        ...
        this.currentAnswerElements.forEach((element, idx) => {
          if (element.answer === correctAnswer) {
            this.tweens.add({ 
              ...
              repeat: 3, 
              onStart: () => { 
                element.bg.setFillStyle(0x68D391); 
              } 
            });
          }
        });
      ...
      showCleanInstruction() {
        ...
        const instruction = this.add.text(this.sys.canvas.width/2, instructionY, 'Fly through the correct coin to answer!', { 
          ...
          fontFamily: 'Arial, sans-serif', 
          stroke: '#1A202C', 
          strokeThickness: Math.max(1, 2 * this.scaleFactor) 
        }).setOrigin(0.5);
        this.tweens.add({ 
          targets: instruction, 
          alpha: 0.7, 
      ...
      createEndScreen(passed) {
        ...
        // Full responsive card sizing - fits all screens perfectly
  ...
  const RotatePrompt = () => React.createElement('div', {
    ...
    }, 'This game is optimized for landscape orientation. Please rotate your device for the best experience.'),
